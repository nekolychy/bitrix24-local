...
jn.define('stafftrack/check-in/pages/check-in', (require, exports, module) => {
	...
	const { AppRatingClient } = require('stafftrack/app-rating-client');
	...
	class CheckInPage extends PureComponent
	{
		...
		async startWorkingDay()
		{
			...
			this.closeLayout(() => {
				AppRatingClient.increaseCheckinCounterAndTryOpenAppRating();
			});
		...
		closeLayout(handler)
		{
			this.refs.layoutWidget?.close(handler);
		}
