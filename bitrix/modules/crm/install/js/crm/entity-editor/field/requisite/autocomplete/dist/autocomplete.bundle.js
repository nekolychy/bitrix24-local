...
(function (exports, main_core, main_core_events, ui_designTokens, ui_feedback_form, ui_common, ui_dropdown, ui_buttons, ui_forms, ui_notification_center, main_loader, crm_placement_detailsearch, ui_dialogs_messagebox) {
	...
	class RequisiteAutocompleteField extends main_core_events.EventEmitter {
		...
		layout(container) {
			...
			this._domNodes.requisiteClearButton = main_core.Tag.render`
			<button class="ui-ctl-after ui-ctl-icon-clear" onclick="${this.onSearchStringClear.bind(this)}"></button>`;
			this._domNodes.requisiteSearchButton = main_core.Tag.render`
			...
			this._domNodes.requisiteSearchString = main_core.Tag.render`
			<input type="text" placeholder="${main_core.Text.encode(placeholder)}" class="ui-ctl-element ui-ctl-textbox" />`;
				...
				if (this.detailAutocompletePlacement) {
					...
					if (main_core.Type.isInteger(params["appId"]) && params["appId"] > 0) {
						if (!this.externalSearchResultHandlerList.has(params["appId"])) {
							this.externalSearchResultHandlerList.set(params["appId"], result => {
								resolve(result);
							});
						}
						return;
		...
		initPlacement(searchControl, container) {
			if (!this.detailAutocompletePlacement) {
				let isAutocompletePlacementEnabled = main_core.Type.isPlainObject(this.clientResolverPlacementParams) && this.clientResolverPlacementParams.hasOwnProperty("numberOfPlacements") && main_core.Type.isNumber(this.clientResolverPlacementParams["numberOfPlacements"]) && this.clientResolverPlacementParams["numberOfPlacements"] > 0;
	...
	class Dropdown extends BX.UI.Dropdown {
		...
		getPopupAlertContainer() {
			if (!this.popupAlertContainer) {
				...
				if (this.isDefaultAppCanInstall()) {
					const appTitleText = main_core.Text.encode(this.getDefaultAppInfo()["title"]);
					const appInstallText = main_core.Text.encode(main_core.Loc.getMessage('REQUISITE_AUTOCOMPLETE_INSTALL'));
					items.push(main_core.Tag.render`
				...
				if (feedbackAvailable) {
					const textParts = main_core.Loc.getMessage('REQUISITE_AUTOCOMPLETE_ADVICE_NEW_SERVICE').split('#ADVICE_NEW_SERVICE_LINK#');
					const item = main_core.Tag.render`<div class="crm-rq-popup-item crm-rq-popup-item-helper"></div>`;
					if (textParts[0] && textParts[0].length) {
						main_core.Dom.append(document.createTextNode(textParts[0]), item);
					}
					const newServiceLinkText = main_core.Loc.getMessage('REQUISITE_AUTOCOMPLETE_ADVICE_NEW_SERVICE_LINK');
					main_core.Dom.append(main_core.Tag.render`<a href="" onclick="${this.showFeedbackForm.bind(this)}">${newServiceLinkText}</a>`, item);
				...
				this.popupAlertContainer = items.length ? main_core.Tag.render`
				<div class="crm-rq-popup-wrapper">
					<div class="crm-rq-popup-items-list">${items}</div>
				</div>
			` : main_core.Tag.render`<div></div>`;
		...
		searchItemsByStr(target) {
			if (this.externalSearchHandler) {
				return this.externalSearchHandler().then(result => {
					return result;
				});
			}
		...
		onClickInstallDefaultApp(event) {
			...
			if (this.isDefaultAppCanInstall()) {
				...
				BX.loadExt('marketplace').then(() => {
					...
					top.BX.addCustomEvent(top, "Rest:AppLayout:ApplicationInstall", this.afterInstallDefaultAppHandler);
		...
		awaitHandler(context) {
			this.wait(context.waitTime).then(() => {
				this.checkDefAppHandler().then(() => {
...
})(this.BX.Crm = this.BX.Crm || {}, BX, BX.Event, BX, BX.UI.Feedback, BX, BX, BX.UI, BX, BX, BX, BX.Crm.Placement, BX.UI.Dialogs);
