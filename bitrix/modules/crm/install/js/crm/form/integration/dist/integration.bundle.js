...
(function (exports, ui_sidepanelContent, main_core, main_core_events, crm_form_type, crm_form_fields_mapper, ui_alerts, ui_buttons, ui_dropdown, main_core_ajax, main_loader, seo_ads_login, ui_dialogs_messagebox) {
	 ...
	 */
	class Integration extends main_core_events.EventEmitter {
		#container;
		#profileContainer;
		#pagesContainer;
		#formsContainer;
		...
		render() {
			...
			if (currentCase && currentCase.linkDirection === 0) {
				...
				this.#container.appendChild(new ui_alerts.Alert({
				...
				this.#container.appendChild(new ui_buttons.Button({
		...
		#renderProfileSelector() {
			if (!this.#profileContainer) {
				this.#profileContainer = main_core.Tag.render`<div></div>`;
			}
			this.#profileContainer.innerHTML = '';
			const provider = this.getProvider();
			if (!provider.profile) {
				this.#profileContainer.appendChild(new ui_alerts.Alert({
					color: ui_alerts.Alert.Color.PRIMARY,
					text: `
					<div class="ui-slider-heading-3">
						${main_core.Loc.getMessage('CRM_FORM_INTEGRATION_JS_LOGIN_TITLE', {
					'%providerName%': this.getTypeTitle()
				})}
					</div>
					<p class="ui-slider-paragraph-2">
						${main_core.Loc.getMessage('CRM_FORM_INTEGRATION_JS_LOGIN_DESC', {
					'%providerName%': this.getTypeTitle()
				})}
					</p>
				`
				}).render());
				this.#profileContainer.appendChild(new ui_buttons.Button({
			...
			this.#profileContainer.appendChild(main_core.Tag.render`
			<div>
				<div class="crm-ads-conversion-block">
					<div class="crm-ads-conversion-social crm-ads-conversion-social-facebook"  style="padding-bottom: 15px; height: 58px;">
						...
						<div class="crm-ads-conversion-social-user">
							<a
								${provider.profile.url ? 'href="' + main_core.Tag.safe`${provider.profile.url}` + '"' : ""}
								target="_top"
								class="crm-ads-conversion-social-user-link"
								>
								${main_core.Tag.safe`${provider.profile.name}`}
							</a>
						</div>
		...
		showBannerForOldProfile() {
			const message = ui_dialogs_messagebox.MessageBox.create({
		...
		showAvatar(provider) {
			if (provider.profile.picture !== undefined) {
				...
				<div
					...
					style="background-image: url(${main_core.Tag.safe`${provider.profile.picture}`})"
				>
				</div>
			</div>`;
			}
		...
		#renderPageSelector() {
			if (!this.#pagesContainer) {
				this.#pagesContainer = main_core.Tag.render`<div></div>`;
			}
			...
			const container = main_core.Tag.render`<div class="crm-form-integration-page-container"></div>`;
			const selectorContainer = main_core.Tag.render`<div class="crm-form-integration-page-selector"></div>`;
		...
		#renderFormSelector() {
			if (!this.#formsContainer) {
				this.#formsContainer = main_core.Tag.render`<div></div>`;
			}
			...
			if (this.getProvider().hasPages) {
				...
				if (!accountId) {
					this.#formsContainer.appendChild(new ui_alerts.Alert({
						color: ui_alerts.Alert.Color.PRIMARY,
			...
			if (this.#adForms.length === 0) {
				this.#formsContainer.appendChild(new ui_alerts.Alert({
					color: ui_alerts.Alert.Color.PRIMARY,
					text: this.#adFormsErrors.length > 0 ? this.#adFormsErrors[0].message : main_core.Loc.getMessage('CRM_FORM_INTEGRATION_JS_FORM_EMPTY', {
						'%providerName%': this.getTypeTitle()
					})
				}).render());
				return this.#formsContainer;
		...
		#renderLoader() {
			const container = main_core.Tag.render`<div style="position: relative; min-height: 100px;"></div>`;
			new main_loader.Loader().show(container).then(() => {});
			return container;
		}
		#renderMapper() {
			if (!this.#mapperContainer) {
				this.#mapperContainer = main_core.Tag.render`<div></div>`;
			}
			this.#mapperContainer.innerHTML = '';
			if (!this.getAdForm()) {
				this.#mapperContainer.appendChild(new ui_alerts.Alert({
					color: ui_alerts.Alert.Color.PRIMARY,
					text: main_core.Loc.getMessage('CRM_FORM_INTEGRATION_JS_FORM_CHOOSE')
				}).render());
				return this.#mapperContainer;
			}
			const mappingMessageContainer = main_core.Tag.render`<div style="margin-bottom: 29px"></div>`;
			new ui_alerts.Alert({
				color: ui_alerts.Alert.Color.PRIMARY,
			...
			const mapper = new crm_form_fields_mapper.Mapper({
				from: {
					caption: this.getTypeTitle()
				},
				fields: this.fields,
				map: this.getAdForm().fields.map(field => {
					let outputCode = this.getCase().fieldsMapping.filter(item => item.adsFieldKey === field.key)[0];
					outputCode = (outputCode || {}).crmFieldKey || '';
					if (!outputCode) {
						outputCode = this.getProvider().defaultMapping.filter(item => {
			...
			const emitChangeEvent = () => {
				const eventFields = [];
				this.getCase().fieldsMapping = mapper.getMap().map(item => {
					if (item.outputCode) {
						eventFields.push({
...
})(this.BX.Crm.Form = this.BX.Crm.Form || {}, BX.UI.Sidepanel.Content, BX, BX.Event, BX.Crm.Form, BX.Crm.Form.Fields, BX.UI, BX.UI, BX, BX, BX, BX.Seo.Ads, BX.UI.Dialogs);
