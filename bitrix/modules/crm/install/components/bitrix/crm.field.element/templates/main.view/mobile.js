...
(function (main_core) {
	...
	let nodeElementCrm = function () {
		...
		nodeElementCrm.prototype = {
			getUrls() {
				let listUrl = BX.message('SITE_DIR') + 'mobile/index.php?mobile_action=get_element_crm_list&' + this.encodeQueryData(this.availableTypes);
				return {
					"list": listUrl,
	...
	BX.Mobile.Field.ElementCrm.prototype = {
		...
		bindElement: function (node) {
			let result = null;
			if (BX(node)) {
				result = new nodeElementCrm(node, BX(`${node.id}_select`), this.useOnChangeEvent, this.availableTypes);
			}
			return result;
		}
	};
