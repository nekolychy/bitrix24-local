	...
	class DedupeAutosearch {
					...
					callback: params => {
						if (BX.prop.getInteger(params, 'entityTypeId', 0) === this._entityTypeId) {
							...
							if (this._status === 'MERGING') {
								let notification = ui_notification.UI.Notification.Center.getBalloonById('crm.autosearch.start_merge');
								if (notification) {
									notification.close();
								}
							}
				...
				BX.PULL.subscribe({
					...
					callback: params => {
						if (BX.prop.getInteger(params, 'entityTypeId', 0) === this._entityTypeId) {
							...
							this._status = BX.prop.getInteger(data, 'CONFLICT_COUNT', 0) > 0 ? 'CONFLICTS_RESOLVING' : '';
		...
		showSettings() {
			...
			if (this.needLoadConflictsCount()) {
				this.loadConflictsCount().then(conflictsCount => {
		...
		showStartConfirmation() {
			...
			const notificationContent = main_core.Tag.render`
			<span>
				${main_core.Loc.getMessage('CRM_DP_AUTOSEARCH_START_CONFIRMATION_TEXT_' + entityTypeName).replace('#FOUND_ITEMS_COUNT#', foundItemsCount).replace('#TOTAL_ENTITIES_COUNT#', totalEntitiesCount)}
				<br>
				${main_core.Loc.getMessage('CRM_DP_AUTOSEARCH_START_CONFIRMATION_TEXT')}
		...
		showMergeCompleteNotification(data) {
			...
			let message = main_core.Tag.render`<div></div>`;
			const automaticallyFoundText = successCount > 0 ? main_core.Loc.getMessage('CRM_DP_AUTOSEARCH_COMPLETE_TEXT_' + entityTypeName).replace('#FOUND_ITEMS_COUNT#', successCount) : main_core.Loc.getMessage('CRM_DP_AUTOSEARCH_EMPTY_RESULTS_' + entityTypeName);
			main_core.Dom.append(main_core.Tag.render`<div>${automaticallyFoundText}</div>`, message);
			let actions = [];
			let notificationWidth = 400;
			if (conflictsCount > 0) {
				main_core.Dom.append(main_core.Tag.render`<div>${main_core.Loc.getMessage('CRM_DP_AUTOSEARCH_COMPLETE_CONFLICTED_TEXT').replace('#CONFLICTS_COUNT#', conflictsCount)}</div>`, message);
				actions.push({
		...
		getSettingsPopupLoader() {
			const loaderContainer = main_core.Tag.render`<div></div>`;
		...
		getSettingsPopupContent(conflictsCount) {
			const selectedExecInterval = this._intervalsList.reduce((prev, item) => item.value === this._selectedExecInterval ? item.title : prev, '');
			this._selectedExecIntervalNode = main_core.Tag.render`<div class="ui-ctl-element">${selectedExecInterval}</div>`;
			return main_core.Tag.render`
			<div>
				${this.getConflictsInfo(conflictsCount)}
				<p>${main_core.Loc.getMessage('CRM_DP_AUTOSEARCH_SETTINGS_NOTE')}</p>
				<div class="ui-ctl-label-text">${main_core.Loc.getMessage('CRM_DP_AUTOSEARCH_SETTINGS_INTERVAL_TITLE')}</div>
		...
		getConflictsInfo(conflictsCount) {
			...
			<div class="ui-alert ui-alert-warning">
				<span class="ui-alert-message">
					${main_core.Loc.getMessage('CRM_DP_AUTOSEARCH_SETTINGS_CONFLICTS_FOUND').replace('#CONFLICTS_COUNT#', conflictsCount)}
					<span class="ui-link" onclick="${this.onStartMergeButtonClick.bind(this)}">${main_core.Loc.getMessage('CRM_DP_AUTOSEARCH_COMPLETE_RESOLVE_CONFLICT_BUTTON')}</span>
				</span>
			</div>`;
		}
		getShortTimeout() {
		...
		openDedupeList() {
			let url = BX.util.add_url_param(this._dedupeListUrl, {
				'is_automatic': 'yes'
			});
			BX.Crm.Page.openSlider(url);
		}
		openMerger() {
			let url = BX.util.add_url_param(this._mergerUrl, {
				'is_automatic': 'yes'
			});
			BX.Crm.Page.openSlider(url);
		...
		closeDropdownMenu() {
			...
			let menu = main_popup.MenuManager.getMenuById('autosearch-settings-intervals-dropdown');
			if (menu) {
		...
		onExternalEvent(event) {
			...
			if (eventName === "BX.Crm.onCrmEntityAutosearchStartMerge" || eventName === "BX.Crm.onCrmEntityAutosearchMergeStatusNotify") {
				...
				if (eventName === "BX.Crm.onCrmEntityAutosearchStartMerge") {
					if (instanceId !== this._instanceId && this._internalMergeStatus !== 'waiting')
		...
		onStartMergeButtonClick() {
			const popup = main_popup.PopupManager.getPopupById(this._settingsPopupId);
		...
		onSelectInterval(interval) {
			...
			if (main_core.Type.isDomNode(this._selectedExecIntervalNode)) {
				this._selectedExecIntervalNode.textContent = this._intervalsList.reduce((prev, item) => item.value === this._selectedExecInterval ? item.title : prev, '');
			}
			this.closeDropdownMenu();
		}
		onHintClick() {
