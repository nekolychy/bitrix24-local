/* eslint-disable */
(function (main_core, crm_terminal, main_core_events, ui_dialogs_messagebox) {
	'use strict';
	...
	class TerminalPaymentList {
		...
		deletePayment(id) {
			ui_dialogs_messagebox.MessageBox.confirm(main_core.Loc.getMessage('CRM_TERMINAL_PAYMENT_LIST_COMPONENT_TEMPLATE_TITLE_DELETE_CONTENT'), (messageBox, button) => {
				button.setWaiting();
			...
			}, main_core.Loc.getMessage('CRM_TERMINAL_PAYMENT_LIST_COMPONENT_TEMPLATE_BUTTON_CONFIRM'), messageBox => messageBox.close(), main_core.Loc.getMessage('CRM_TERMINAL_PAYMENT_LIST_COMPONENT_TEMPLATE_BUTTON_BACK'));
		}
		deletePayments() {
		...
		openTerminalSettingsSlider(event, menuItem) {
		...
		openPaymentInSalescenter(params) {
			const options = {
...
})(BX, BX.Crm, BX.Event, BX.UI.Dialogs);
