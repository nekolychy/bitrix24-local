...
if (WIZARD_INSTALL_DEMO_DATA || WIZARD_B24_TO_CP)
{
	...
	$arNewUrlRewrite = array(
		...
		[
			'CONDITION' => "#^" . WIZARD_SITE_DIR . "bi/table/#",
			'RULE' => "",
			'ID' => 'bitrix:biconnector.apachesuperset.workspace_analytic.controller',
			'PATH' => WIZARD_SITE_DIR . 'bi/table/index.php',
		],
		[
		...
		[
			'CONDITION' => '#^' . WIZARD_SITE_DIR . 'disk/file/(?<unique_code>[0-9a-zA-Z]{20})/edit(\/?)#',
			'RULE' => 'action=disk.api.unifiedlinkcontroller.edit&uniqueCode=$1&',
			'PATH' => WIZARD_SITE_DIR . 'bitrix/services/main/ajax.php',
		],
		[
			'CONDITION' => '#^' . WIZARD_SITE_DIR . 'disk/file/(?<unique_code>[0-9a-zA-Z]{20})(\/?)#',
		...
		[
			"CONDITION" => "#^" . WIZARD_SITE_DIR . "pub/annuals/([0-9a-zA-Z~]+)/#",
			"RULE" => "shortCode=$1",
			"PATH" => WIZARD_SITE_DIR . "/pub/annual_summary.php",
			"SORT" => 50,
		],
		[
			"CONDITION" => "#^" . WIZARD_SITE_DIR . "pub/annual_summary/([0-9a-z\.]+)/([0-9a-z\.\_]+)/\?user_lang=[a-z]+#",
			"RULE" => "signedId=$1&signedType=$2",
			"PATH" => WIZARD_SITE_DIR . "pub/annual_summary.php",
			"SORT" => 60,
		],
		[
			"CONDITION" => "#^" . WIZARD_SITE_DIR . "desktop/menu/#",
			"RULE" => "",
			"ID" => "bitrix:intranet.menu",
			"PATH" => WIZARD_SITE_DIR . "desktop_menu/index.php",
		],
		[
	...
	if (!in_array('intranet.php', $config, true))
	{
		$config[] = 'intranet.php';
