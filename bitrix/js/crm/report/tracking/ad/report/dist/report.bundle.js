...
(function (exports, sidepanel, ui_progressbar, ui_fonts_opensans, main_core, main_core_events, main_popup) {
	...
	class Report {
		...
		loaded = false;
		statusButtonClassName = 'crm-tracking-report-source-status-disabled';
		constructor(options) {
			this.load(options);
		}
		static open(options) {
		...
		load({
			...
			BX.SidePanel.Instance.open('crm:api.tracking.ad.report' + `-${sourceId}-${level}`, {
				cacheable: false,
				contentCallback: () => {
		...
		loadGrid() {
			BX.ajax.runAction('crm.api.tracking.ad.report.getGrid', {
				...
				main_core_events.EventEmitter.subscribe(window, 'Grid::beforeRequest', this.onBeforeGridRequest.bind(this));
				main_core.Runtime.html(this.getNode('grid'), data.html);
				this.initActivators();
				this.hideLoader();
				this.loaded = true;
				if (!this.filter.level) {
		...
		initActivators() {
			this.getNodes('grid/activator').forEach(node => {
				...
				if (statusBtn) {
					options.enabled ? statusBtn.classList.remove(this.statusButtonClassName) : statusBtn.classList.add(this.statusButtonClassName);
					main_core.Event.bind(statusBtn, 'click', () => {
						const popup = new main_popup.Menu({
			...
			if (selector) {
				...
				main_core.Event.bind(selector, 'click', () => {
						...
						<div data-role="loader/bar" class="crm-report-tracking-panel-loader-bar">
							<div data-role="error" class="ui-alert ui-alert-danger" style="display: none;">
								<span class="ui-alert-message">
									<strong>${main_core.Loc.getMessage('CRM_REPORT_TRACKING_AD_REPORT_ERROR_TITLE')}:</strong>
							...
							<div style="text-align: center;">
								<button 
									...
									style="display: none;"
								>${main_core.Loc.getMessage('CRM_REPORT_TRACKING_AD_REPORT_CLOSE')}</button>
							</div>
						</div>
					</div>
