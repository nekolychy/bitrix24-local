...
(function (exports, crm_ai_nameService, main_core, ui_textEditor, ui_vue3, pull_queuemanager, ui_notification, ui_buttons, ui_iconSet_main, ui_bbcode_parser, main_core_events, ui_entitySelector, ui_infoHelper, ui_forms, crm_timeline_tools, main_date, ui_datePicker) {
	'use strict';
	...
	class PromptLength {
		...
		validate(prompt) {
			if (prompt.length < LENGTH_RANGE.MIN) {
				this.#errorMessage = main_core.Loc.getMessage('CRM_COPILOT_CALL_ASSESSMENT_PAGE_ABOUT_PROMPT_LENGTH_MIN_ERROR', {
					'#MIN_VALUE#': LENGTH_RANGE.MIN
				});
				return false;
			}
			if (prompt.length > LENGTH_RANGE.MAX) {
				this.#errorMessage = main_core.Loc.getMessage('CRM_COPILOT_CALL_ASSESSMENT_PAGE_ABOUT_PROMPT_LENGTH_MAX_ERROR', {
	...
	const AboutPage = {
		...
		created() {
			this.parser = new ui_bbcode_parser.BBCodeParser();
			this.promptLengthValidator = new PromptLength();
		},
		methods: {
			getData() {
				return {
	...
	const ClientPage = {
		...
		methods: {
			...
			getClientTypeClassList(clientTypeId) {
				...
				return ['crm-copilot__call-assessment_page-section-body-field-client-type', `--client-${clientType.id}`, {
	...
	class BordersAccord {
		...
		validate(lowBorder, highBorder) {
			if (highBorder < lowBorder) {
				this.#errorMessage = main_core.Loc.getMessage('CRM_COPILOT_CALL_ASSESSMENT_PAGE_CONTROL_VALIDATION_ERROR');
				return false;
			}
			this.#errorMessage = null;
			return true;
	...
	class TimePeriod {
		...
		validate(startTime, endTime) {
			...
			if (Number.isNaN(start) || Number.isNaN(end)) {
				this.#errorMessage = main_core.Loc.getMessage('CRM_COPILOT_CALL_ASSESSMENT_PAGE_SETTINGS_VALIDATION_TIME_ERROR_1');
				return false;
			}
			if (start.getTime() >= end.getTime()) {
				this.#errorMessage = main_core.Loc.getMessage('CRM_COPILOT_CALL_ASSESSMENT_PAGE_SETTINGS_VALIDATION_TIME_ERROR_2');
				return false;
			}
			this.#errorMessage = null;
			return true;
	...
	const CustomRow = {
		...
		data() {
			return {
				currentWeekdayType: this.weekdayType ?? weekdayType.working,
				currentStartPoint: this.startPoint,
				currentEndPoint: this.endPoint,
		...
		created() {
			this.timePeriodValidator = new TimePeriod();
			this.datePicker = new ui_datePicker.DatePicker({
				type: 'time',
				dateFormat: crm_timeline_tools.DatetimeConverter.getSiteDateTimeFormat(true),
				timeFormat: crm_timeline_tools.DatetimeConverter.getSiteShortTimeFormat(),
		...
		watch: {
			currentWeekdayType(weekdayType) {
	...
	const TypeSelector = {
		...
		watch: {
			currentAvailabilityType(availabilityType) {
	...
	const SettingsPage = {
		...
		data() {
			...
			return {
				...
				availabilityType: this.data.availabilityType ?? availabilityType.always_active,
		...
		methods: {
			...
			sendData() {
				...
				main_core.ajax.runAction('crm.copilot.callassessment.save', {
					...
					const messageCode = response.errors[0]?.code === 'AI_NOT_AVAILABLE' ? 'CRM_COPILOT_CALL_ASSESSMENT_PAGE_COPILOT_ERROR' : 'CRM_COPILOT_CALL_ASSESSMENT_PAGE_SAVE_ERROR';
					ui_notification.UI.Notification.Center.notify({
			...
			onSaveCallback() {
				if (main_core.Type.isFunction(this.params?.events?.onSave)) {
	...
	class CallAssessment {
		...
		constructor(containerId, params = {}) {
			...
			this.#app = ui_vue3.BitrixVue.createApp(CallAssessment$1, {
		...
		#initTitleInlineEditing(params) {
			...
			if (main_core.Type.isString(data?.title)) {
		...
		#getTextEditor({
			...
			const toolbar = this.#isReadOnly ? [] : ['bold', 'italic', 'underline', 'strikethrough', '|', 'numbered-list', 'bulleted-list', 'copilot'];
