...
(function (exports, main_core) {
	...
	class Loader {
		...
		constructor(placementCode, settings) {
			this.placementCode = main_core.Type.isStringFilled(placementCode) ? placementCode : "";
			...
			if (placement) {
				...
				BX.ajax.runComponentAction("bitrix:app.placement", "getComponent", {
					data: {
		...
		executeCallbacks() {
			...
			let callback;
			while ((callback = this.queue.shift()) && main_core.Type.isFunction(callback)) {
				callback({
		...
		static getCallbackFromSettings(settings) {
			let callback = null;
			if (main_core.Type.isPlainObject(settings) && settings.hasOwnProperty("callback") && main_core.Type.isFunction(settings["callback"])) {
		...
		static create(placementCode, settings) {
			const copiedSettings = main_core.Runtime.clone(settings);
	...
	class DetailSearchItem {
		...
		find() {
			...
			BX.ajax.runComponentAction("bitrix:app.layout", "getComponent", {
				data: data
				...
				const iframeNode = BX.create("DIV", {
					attrs: {
						"data-app-sid": componentResult["APP_SID"]
					},
					style: {
						"display": "none",
						overflow: "hidden"
	...
	class DetailSearch {
		...
		constructor(placementCode) {
			...
			Loader.create(this.placementCode, {
		...
		init(data) {
			...
			if (data.success === true && data.placement && data.placement.param && data.placement.param.extendedList instanceof Map) {
				...
				const isNeedInitializeInterface = isPlacementInterface && !data["placementInterface"].hasOwnProperty("crmShowFoundEntities");
				if (isPlacementInterface) {
					this.placementInterface = isNeedInitializeInterface ? this.initializeInterface(data["placementInterface"]) : data["placementInterface"];
				}
			}
			this.isInited = true;
			if (this.isShown) {
		...
		getCountryMap(placementData) {
			...
			const countriesOption = BX.prop.getString(BX.prop.getObject(placementData, "options", {}), "countries", "");
			if (main_core.Type.isStringFilled(countriesOption) && /^[1-9][0-9]*(,[1-9][0-9]*)*$/.test(countriesOption)) {
				countriesOption.split(',').forEach(countryCode => {
		...
		showMenu() {
			...
			BX.adjust(this.container, {
				props: {
					className: "placement-container menu-popup"
				}
			});
			this.replaceEvents();
			this.destroyMenu();
			...
			if (main_core.Type.isObject(this.placementParams) && this.placementParams["isPlacement"] && this.placementParams["numberOfPlacements"] <= 1) {
		...
		restCrmShowCreatedEntity(applayout, createdEntity) {
			this.activeProcesses--;
			BX.onCustomEvent(this, "Placements:select", [createdEntity]);
		}
		...
		getFirstPlacementMenuItem() {
			...
			if (main_core.Type.isStringFilled(firstMenuItemId) && this.menu) {
		...
		refreshContainerVisibility(params) {
			...
			const isVisible = isPlacementsAvailable && (!isPlacement || isPlacement && numberOfPlacements > 1) && currentSearchQuery.length > 2;
		...
		activateFirstPlacementMenuItem() {
			setTimeout(() => {
				const menuItem = this.getFirstPlacementMenuItem();
				if (menuItem) {
					menuItem.layout.text.dispatchEvent(new MouseEvent("click", {
						bubbles: true,
						cancelable: true
					}));
				}
		...
		onStartDefaultSearch(params) {
			if (main_core.Type.isPlainObject(params) && params.hasOwnProperty("appId")) {
	...
	exports.DetailSearch = DetailSearch;

})(this.BX.Crm.Placement = this.BX.Crm.Placement || {}, BX);
//# sourceMappingURL=placement.bundle.js.map
