...
(function (exports, main_core) {
	...
	class BatchMergeManager {
		...
		getMessage(name) {
			return this.getMessageFromSettings(name) ?? this.getMessageFromLocalization(name);
		}
		getEntityIds() {
		 ...
		 * @returns {string|null}
		 */
		getMessageFromLocalization(name) {
			const phraseCode = 'CRM_BATCH_MERGER_MANAGER_#ENTITY_TYPE_NAME#_#NAME#'.replace('#ENTITY_TYPE_NAME#', this.getMessageEntityTypeName()).replace('#NAME#', name.toUpperCase());
			return main_core.Loc.getMessage(phraseCode);
		}
		getMessageEntityTypeName() {
			const entityTypeName = BX.CrmEntityType.isDynamicTypeByTypeId(this.getEntityTypeId()) ? BX.CrmEntityType.names.dynamic : BX.CrmEntityType.resolveName(this.getEntityTypeId());
		...
		#getMergerUrl() {
			...
			const uri = new main_core.Uri(mergerBaseUrl);
			uri.setQueryParams({
