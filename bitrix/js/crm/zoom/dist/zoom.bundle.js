...
(function (exports, main_core, main_core_events, calendar_planner, calendar_util) {
	...
	class Zoom {
		...
		constructor(params) {
			...
			main_core.Event.bind(this.getDateContainer(), 'click', e => {
				this.onDateFieldClick(e);
			});
			main_core.Event.bind(this.getTimeContainer(), 'click', () => {
				this.onTimeSwitchClick(this.getTimeInputField());
			});
			main_core.Event.bind(this.getDateContainer(), 'change', () => {
				this.onUpdateDateTime();
			});
			main_core.Event.bind(this.getTimeContainer(), 'change', () => {
				this.onUpdateDateTime();
			});
			main_core.Event.bind(this.getDurationInputField(), 'change', () => {
				this.onUpdateDateTime();
			});
			main_core.Event.bind(this.getDurationTypeInputField(), 'change', () => {
				this.onUpdateDateTime();
			});
		...
		getTitleInputField() {
			return this.cache.remember('titleInputField', () => {
				return main_core.Tag.render`
				<input type="text" class="ui-ctl-element" value="${main_core.Loc.getMessage('CRM_ZOOM_NEW_CONFERENCE_TITLE_PLACEHOLDER')}">
			`;
			});
		}
		getDurationInputField() {
		...
		getDurationTypeInputField() {
			return this.cache.remember('durationTypeInputField', () => {
				return main_core.Tag.render`
				<select class="ui-ctl-element">
					<option value="m">${main_core.Loc.getMessage('CRM_ZOOM_NEW_CONFERENCE_DURATION_MINUTES')}</option>
					<option value="h">${main_core.Loc.getMessage('CRM_ZOOM_NEW_CONFERENCE_DURATION_HOURS')}</option>
				</select>
			`;
			});
		}
					...
					<div class="crm-entity-stream-content-new-zoom-inner">
						<div class="crm-entity-stream-content-new-zoom-field">
							<div class="crm-entity-stream-content-new-zoom-field-inner">
								<label for="" class="crm-entity-stream-content-new-zoom-field-label">${main_core.Loc.getMessage('CRM_ZOOM_NEW_CONFERENCE_TITLE')}</label>
						...
						<div class="crm-entity-stream-content-new-zoom-field">
							<div class="crm-entity-stream-content-new-zoom-field-block">
								<label for="" class="crm-entity-stream-content-new-zoom-field-label">${main_core.Loc.getMessage('CRM_ZOOM_NEW_CONFERENCE_DATE_CAPTION')}</label>
							...
							<div class="crm-entity-stream-content-new-zoom-field-block">
								<label for="" class="crm-entity-stream-content-new-zoom-field-label">${main_core.Loc.getMessage('CRM_ZOOM_NEW_CONFERENCE_DURATION_CAPTION')}</label>
								<div class="crm-entity-stream-content-new-zoom-field-control">
		...
		renderSaveButton() {
			return this.cache.remember('saveButton', () => {
				...
				<button onclick="${this.save.bind(this)}" class="ui-btn ui-btn-xs ui-btn-primary ui-btn-round">
					${main_core.Loc.getMessage('UI_BUTTONS_CREATE_BTN_TEXT')}
				</button>
			`;
			});
		}
		...
		renderCancelButton() {
			return this.cache.remember('cancelButton', () => {
				...
				<span onclick="${this.cancel.bind(this)}" class="ui-btn ui-btn-xs ui-btn-link">
					${main_core.Loc.getMessage('UI_TIMELINE_EDITOR_COMMENT_CANCEL')}
				</span>
			`;
			});
		}
		...
		save() {
			...
			if (!main_core.Type.isString(conferenceTitle) || conferenceTitle === '') {
				this.errorMessages.push(`${main_core.Loc.getMessage('CRM_ZOOM_ERROR_EMPTY_TITLE')}`);
				this.showError();
			}
			if (!main_core.Type.isInteger(timestampStart) || timestampStart < Date.now()) {
				this.errorMessages.push(`${main_core.Loc.getMessage('CRM_ZOOM_ERROR_INCORRECT_DATETIME')}`);
				this.showError();
			}
			if (!main_core.Type.isInteger(duration) || duration <= 0 || !['h', 'm'].includes(durationType)) {
				this.errorMessages.push(`${main_core.Loc.getMessage('CRM_ZOOM_ERROR_INCORRECT_DURATION')}`);
				this.showError();
			}
			if (!this.error) {
				BX.ajax.runAction('crm.api.zoomUser.createConference', {
					...
					this.errorMessages.push(`${main_core.Loc.getMessage('CRM_ZOOM_CREATE_MEETING_SERVER_RETURNS_ERROR')}`);
		...
		showError() {
			...
			if (!this.error && errorText !== '') {
				this.errorElement = main_core.Tag.render`<div class="zoom-error-message ui-alert ui-alert-danger ui-alert-icon-danger">
