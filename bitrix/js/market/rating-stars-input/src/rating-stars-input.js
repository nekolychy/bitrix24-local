...
export const RatingStarsInput = {
	...
	computed: {
		...
		value()
		{
			const valueNumber = Text.toNumber(this.modelValue);
	...
	methods: {
		...
		onStarEnter(star)
		{
			...
			const starNumber = Text.toNumber(star);
			if (starNumber > 0)
			{
		...
		onStarClick(star)
		{
			...
			const starNumber = Math.max(1, Math.min(5, Text.toNumber(star)));
		...
		getSizeValue()
		{
			const size = Text.toNumber(this.size);

			return size > 0 ? size : 33;
		},
		getRootClass()
		{
			if (!Type.isStringFilled(this.className))
			{
				return {};
			}
		...
		{
			const style = {};
			if (Type.isStringFilled(this.emptyStrokeColor))
			{
				style['--market-rating-stars-input-empty-stroke'] = this.emptyStrokeColor;
			}

			if (Type.isStringFilled(this.emptyHoverStrokeColor))
			{
